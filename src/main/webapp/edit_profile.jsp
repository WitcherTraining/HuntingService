...
<head>
    ...
    <title><fmt:message key="head.edit.profile"/></title>
...
<div class="container text-center text-light">
    <div class="col-md-12">
        <form action="EditProfile" method="post" class="align-middle">
            <h2><fmt:message key="head.edit.profile"/> ${sessionScope.login}</h2>
            <p><input type="email" name="email" value="${sessionScope.email}" placeholder="<fmt:message key="label.email"/>"
                      required></p>
            <c:if test="${requestScope.notCorrectEmail != null}">
                <small class="text-danger"><fmt:message key="error.email"/></small>
            </c:if>
            <p><input type="text" name="phone" value="${sessionScope.phone}" placeholder="<fmt:message key="label.phone"/>"
